...
import {
    faFilter, faArrowDownWideShort,
} from "@fortawesome/free-solid-svg-icons";
import "@fortawesome/fontawesome-svg-core/styles.css";
import Table from "../../../../components/Table";
import Link from "next/link";
import { projectsData, access } from "../../../../lib/data";
    ...
    >
        ...
        /> */}
            ...
            <div className="flex flex-col">
                <h3 className="text-sm font-semibold">{item.projectName}</h3>
            </div>
        </td>
        <td className="hidden md:table-cell"> {new Date(item.startDate).toLocaleDateString()}</td>
        <td className="hidden md:table-cell"> {new Date(item.endDate).toLocaleDateString()}</td>
        <td>
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
                    ...
                    <div className="flex items-center gap-4 self-end">
                        ...
                        {access === "admin" && <FormModal table="project" type="create" />}
                    </div>
                </div>
            </div>
            {/* list */}
